...
# more claims to cover part of the same areas. For example, consider 
...
# 2. (Claim 3, while adjacent to the others, does not overlap 
# either of them.)
# 
# If the Elves all proceed with their own plans, none of them 
# will have enough fabric. How many square inches of fabric are 
# within two or more claims?

import re
import csv

path = "C:\chuck\\adventofcode\day 3_1 input.txt"
csvPath = "C:\chuck\\adventofcode\day3_table.csv"
...
cleanerPuzzleInput = []
separatedPuzzleInput = []
coordinateList = []
sizeList = []

for line in puzzleInput:
    cleanerPuzzleInput.append(re.sub('^[^\d].*@', "", line))

for line in cleanerPuzzleInput:
    sizeList.append(re.sub(r".*?(\w+)\s*$", r"\1", line))

for line in cleanerPuzzleInput:
    coordinateList.append(re.sub(":.*", "", line))

number_of_submissions = len(sizeList)
...
for entry in coordinateList:
    print(i)
    thisStartingPoint = coordinateList[i]
    thisSize = sizeList[i]

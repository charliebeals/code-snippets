...
# the Temporal Anomaly Research and Detection Instrument Station tells you. 
...
FrequencyFile = "C:\chuck\Advent Of Code\day 1_1 input.txt"

with open(FrequencyFile) as f:
    content = f.readlines()
contentList = [line.rstrip('\n') for line in content]

masterFrequency = 0

for frequency in contentList:
    operator = frequency[0]
    value = int(frequency[1:])
    if operator == "-":
        masterFrequency = masterFrequency - value
    else:
        masterFrequency = masterFrequency + value
    
print(masterFrequency)